angular.module('starter.services', [])
  ...
  .factory('chartDataCacheService', function(CacheFactory) {
    ...
    if (!CacheFactory.get('charDataCache')) {
      charDataCache = CacheFactory('charDataCache', {
        maxAge         : 60 * 60 * 8 * 1000,
        deleteOnExpire : 'aggressive',
        storageMode    : 'localStorage'
      });
    } else {
      charDataCache = CacheFactory.get('charDataCache');
    }
  ...
  .factory('stockDetailsCacheService', function(CacheFactory) {
    ...
    if (!CacheFactory.get('stockDetailsCache')) {
      ...
      charDataCache = CacheFactory.get('stockDetailsCache');
    }
  ...
  .factory('stockDataService', function($q, $http, encodeURIService, stockDetailsCacheService) {
    ...
    var getPriceData = function(ticker) {
      var deferred = $q.defer(),
          ...
          ticker + '/quote?format=json&view=detail';
  ...
  .factory('chartDataService', function($q, $http, encodeURIService, chartDataCacheService) {
    var historicalData = function(ticker, fromDate, todayDate) {
      var deferred  = $q.defer(),
          cacheKey  = ticker,
        ...
        .success(function(json) {
          ...
          jsonData.forEach(function(dayDataObject) {
            var dateToMillis  = dayDataObject.Date,
                ...
                volumeDatum.push(date);
                volumeDatum.push(volume);
                priceDatum.push(date);
                priceDatum.push(parseFloat(price));
